...
诸如“非常满意”、“满意”、“一般”、“不满意”、“非常不满意”的量表题，往往会以“5”、“4”、“3”、“2”、“1”等数值储存在数据中。
因此，“编码”往往是是我们处理问卷数据的第一步。那就利用今天所学，使用示例的WVS7数据进行编码练习吧。请完成下列要求：

读取WVS7数据，将其储存在R的内存中，并命名为“data”。将“data”中的“education”变量作为**类别向量**储存在名为“a”的对象中，类别名称使用联合国教科文组织所采用的“ISCED-2011”(0-幼儿教育，1-小学教育，2-初中教育，3-高中教育，4-中学后非高等教育，5-短周期高等教育，6-学士或同等学历，7-硕士或同等学历，8-博士或同等学历)。
在完成后将a转换成数据框，并写入"education.csv"文件中。
...
a <- factor(data$education, levels = c(0, 1, 2, 3, 4, 5, 6, 7, 8), 
                   labels = c("幼儿教育", "小学教育", "初中教育", "高中教育", "中学后非高等教育", "短周期高等教育", "学士或同等学历", "硕士或同等学历", "博士或同等学历"))

write_csv(as.data.frame(a), "data.csv")
